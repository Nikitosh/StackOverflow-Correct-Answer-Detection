...
class textstatistics:
    def __init__(self):
        ...
        self.easy_word_set = set([ln.strip() for ln in open('textstat/easy_words.txt')])

    @repoze.lru.lru_cache(maxsize=128)
    def letter_count(self, text):
        """
        Function to return total letter counts in a text
        """
        return sum(1 for c in text if c.isalpha())

    @repoze.lru.lru_cache(maxsize=128)
    def uppercase_letter_count(self, text):
        """
        Function to return total uppercase letter counts in a text
        """
        return sum(1 for c in text if c.isupper())

    @repoze.lru.lru_cache(maxsize=128)
    def lowercase_letter_count(self, text):
        """
        Function to return total lowercase letter counts in a text
        """
        return sum(1 for c in text if c.islower())

    @repoze.lru.lru_cache(maxsize=128)
    def space_count(self, text):
        """
        Function to return total space counts in a text
        """
    ...
    @repoze.lru.lru_cache(maxsize=128)
    def lexicon(self, text, removepunct=True):
        """
        Function to return total lexicon (words in lay terms) from a text
        """
        if removepunct:
            text = ''.join(ch for ch in text if ch not in self.exclude)
    ...
    @repoze.lru.lru_cache(maxsize=128)
    def longest_sentence_char_count(self, text):
        """
        Char count of the longest sentence in a text
        """
        sentences = re.split(r' *[\.\?!][\'"\)\]]* *', text)
        max_length = 0
        for sentence in sentences:
            max_length = max(max_length, self.char_count(sentence, ignore_spaces=False))
        return max_length

    @repoze.lru.lru_cache(maxsize=128)
    def longest_sentence_lexicon_count(self, text):
        """
        Char count of the longest sentence in a text
        """
        sentences = re.split(r' *[\.\?!][\'"\)\]]* *', text)
        max_length = 0
        for sentence in sentences:
            max_length = max(max_length, self.lexicon_count(sentence))
        return max_length
